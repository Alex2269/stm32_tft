    ...
    if(c & 0x80)
    {
      ...
      pFs = getHAN_font(c, c2, c3);
			...
			for(i = 0; i < 16; i++)
			{
				for(bit = 0; bit < 8; bit++)
				{
					if((pFs[i * 2] << bit) & 0x80) LCD_draw_point(XPOS + bit,  YPOS + i, fcolor);
					else if(!Transparent) LCD_draw_point(XPOS + bit,  YPOS + i, bcolor);
				}

				for(bit = 0; bit < 8; bit++)
				{
					if((pFs[i * 2 + 1] << bit) & 0x80) LCD_draw_point(XPOS + 8 + bit,  YPOS + i, fcolor);
					else if(!Transparent) LCD_draw_point(XPOS + 8 + bit,  YPOS + i, bcolor);
				}
			}

      XPOS = XPOS + 16;
      if(XPOS > LCD_GetWidth())
      {
        XPOS = 0;
        YPOS = YPOS + 16;
      }
    ...
    else
    {
			for(k = 0; k < 2; k++)
			{
				for(i = 0; i < 8; i++)
				{
					for(j = 0; j < 8; j++)
					{
						if(E_FONT[c][i + k * 8] & (0x01 << j)) LCD_draw_point(XPOS + i, YPOS + k * 8 + j, fcolor);
						else if(!Transparent) LCD_draw_point(XPOS + i, YPOS + k * 8 + j, bcolor);
					}
				}
			}
