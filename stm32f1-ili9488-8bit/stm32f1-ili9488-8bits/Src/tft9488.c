...
void tft_write_bus(uint8_t byte)
{
  ...
  // TFT   D2 D3 D4 D5 D6 D7 D0 D1  circular shift right of byte on 2 bits, for alignment of wires 
...
void tft_draw_dot_line(uint16_t row1, uint16_t column1, uint16_t row2, uint16_t column2,uint16_t color)
{
  ...
  {
    incx=-4;delta_x=-delta_x;
  }
  if(delta_y>0)incy=4;
  else if(delta_y==0)incy=0;
  else
  {
    incy=-4;delta_y=-delta_y;
  }
  if( delta_x>delta_y)distance=delta_x;
  else distance=delta_y;
  for(t=0;t<=distance+1;t+=4 )
  {
