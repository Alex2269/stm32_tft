...
{
  write_to_buffer_data(data >> 8);
  write_to_buffer_data(data&0x00ff);
}

void
tft_write_data24(uint32_t data)
{
  write_to_buffer_data(data >> 16);
  write_to_buffer_data(data >> 8);
  write_to_buffer_data(data & 0x00ff);
}
...
{
  write_to_buffer_reg(data >> 8);
  write_to_buffer_reg(data&0x00ff);
}
...
void R61581_8bit(void)
{
  ...
  tft_write_reg(0x36);
  tft_write_data(0x48); // mirror X-Y
  LL_mDelay(150);
...
{
  write_to_buffer_reg(0x2a); // SET column address
  // printf ("data: %X, %X, %X, %X\n",data[0],data[1],data[2],data[3]);
  tft_write_data16(s_x);
  tft_write_data16(e_x);
  write_to_buffer_reg(0x2b); // SET row address
