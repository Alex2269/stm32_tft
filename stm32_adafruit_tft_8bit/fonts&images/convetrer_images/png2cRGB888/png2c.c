 ...
 * in either RGB888 or RGB5A1 format. This is useful for
 * embedding pixmaps to display
 */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
int main(int argc, char* argv[]) {
    ...
    fprintf(outputSource,
        ...
        "const int %s_size = %u;\n\n"
        "const unsigned char %s[%u*3+16] = {\n", /*"const unsigned short %s[%u+2] = {\n",*/
        imageName, imageName, width, imageName, height, imageName, width*height, imageName,width*height);

    fprintf(outputSource, " 0x%02x,0x%02x,0x%02x,0x%02x, // width\n", (uint8_t)width,(uint8_t)(width>>8),(uint8_t)(width>>16),(uint8_t)(width>>24));
    fprintf(outputSource, " 0x%02x,0x%02x,0x%02x,0x%02x, // height\n", (uint8_t)height,(uint8_t)(height>>8),(uint8_t)(height>>16),(uint8_t)(height>>24));
    fprintf(outputSource, " 0x%02x,0x%02x, // reserv\n", 0,0);
    fprintf(outputSource, " 0x%02x,0x%02x,0x%02x,0x%02x, // reserv\n", 0,0,0,0);
    fprintf(outputSource, " 0x%02x,0x%02x, // offset\n\n", 0,16);
    ...
    for (y=0; y<height; y++) {
        ...
        for (x=0; x<width; x++) {
            ...
            /** unsigned short convPixel = generate5A1 ?
            RGB8888toRGB5A1(pixel[0], pixel[1], pixel[2], pixel[3]) :
            RGB888toRGB565(pixel[0], pixel[1], pixel[2]); **/

            if( ! (cnt % 1) ) fprintf(outputSource, " ");
            if( ! (cnt % 8) ) fprintf(outputSource, "\n");

            unsigned short convPixel = generate5A1; // ?
            if(generate5A1)
             fprintf(outputSource, "0x%02x,0x%02x,0x%02x,0x%02x,", pixel[0], pixel[1], pixel[2], pixel[3]);
            else
              fprintf(outputSource, "0x%02x,0x%02x,0x%02x,", pixel[0], pixel[1], pixel[2]);
