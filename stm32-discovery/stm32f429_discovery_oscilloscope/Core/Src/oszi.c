...
#define FFT_VISIBLE_LENGTH 320
#define FFT_LENGTH 480

float FFT_DATA_IN[FFT_LENGTH]={0.0};
uint32_t FFT_UINT_DATA[0];

extern GUI_t GUI;
extern Menu_t Menu;
extern uint16_t ADC_DMA_Buffer_A[];  //  (A) Roh-Daten per DMA
extern uint16_t ADC_DMA_Buffer_B[];  //  (B) Roh-Daten per DMA
extern uint16_t ADC_DMA_Buffer_C[];  //  (C) sortierte Daten
extern ADC_t ADC_UB;
