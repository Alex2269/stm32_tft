...
void UG_PutChar(char chr)
{
  ...
  Display_WindowSet(getCursorX(),getCursorX()+actual_char_width-1,yo,yo+font->char_height);
    ...
    for( j=0;j<font->char_height;j++ )
    {
      ...
      for( i=0;i<bn;i++ )
      {
        ...
        for( k=0;(k<8) && c;k++ )
        {
          if( b & 0x01 )
          {
            // writePixel(xo,yo, _fontColor);
            // fillRect(xo,yo,_textsize,_textsize,_fontColor);
            Display_WriteData24(_fontColor);
          }
          else
          {
            // writePixel(xo,yo, _fontBgColor);
            // fillRect(xo,yo,_textsize,_textsize,_fontBgColor);
  ...
  else if (font->font_type == FONT_TYPE_8BPP)
  {
    ...
    for( j=0;j<font->char_height;j++ )
    {
      ...
      for( i=0;i<actual_char_width;i++ )
      {
        ...
        // writePixel(xo,yo, _fontBgColor);
        // fillRect(xo,yo,_textsize,_textsize,color);
