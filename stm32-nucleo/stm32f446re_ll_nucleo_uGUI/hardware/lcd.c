...
#elif defined(R61581)
static const uint8_t R61581_regValues[] = {
  #undef TFTWIDTH
  #undef TFTHEIGHT
  #define TFTWIDTH			320
...
void GPIO_Init(uint32_t mode) {
	...
	if (!(mode == LL_GPIO_MODE_OUTPUT || mode == LL_GPIO_MODE_INPUT)) return;
...
inline uint8_t LCD_Read8Register8(uint8_t a, uint8_t dummy) {
	...
	LCD_CD_COMMAND();
	LCD_Write8(a);
	GPIO_Init(LL_GPIO_MODE_INPUT);
	LCD_CD_DATA();
	if (dummy) LCD_RD_STROBE();
	data = LCD_Read8();
	GPIO_Init(LL_GPIO_MODE_OUTPUT);
	return data;
}
...
#elif defined(R61581)
	uint8_t i = 0;
	while(i < sizeof(R61581_regValues)/sizeof(R61581_regValues[0])) {
		uint8_t r = R61581_regValues[i++];
		uint8_t len = R61581_regValues[i++];
		if(r == TFTLCD_DELAY) {
			HAL_Delay(len);
		} else {
			LCD_CD_COMMAND();
...
#elif defined(ILI9340) || defined(ILI9341) || defined(ILI9341_00) || defined(R61520) || defined(R61581) || defined(UNKNOWN1602)	
...
void LCD_SetRotation(uint8_t x) {
    ...
    LCD_SetAddrWindow(0, 0, m_width - 1, m_height - 1);
#elif defined(ILI9340) || defined(ILI9341) || defined(ILI9341_00) || defined(R61520) || defined(R61581)
    uint8_t t;
    switch (m_rotation) {
