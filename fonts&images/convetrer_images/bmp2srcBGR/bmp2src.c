...
int main( int argc, char ** argv )
{
  ...
  // SizeBMP = bmp[0x02] | bmp[0x02+1]<<8 | bmp[0x02+2]<<16 | bmp[0x02+3]<<24;
  // offset = bmp[0x0a] | bmp[0x0a+1]<<8 | bmp[0x0a+2]<<16 | bmp[0x0a+3]<<24;
  w = bmp[0x12] | bmp[0x12+1]<<8 | bmp[0x12+2]<<16 | bmp[0x12+3]<<24;
  h = bmp[0x16] | bmp[0x16+1]<<8 | bmp[0x16+2]<<16 | bmp[0x16+3]<<24;
  bits = bmp[0x1C] | bmp[0x1C+1]<<8;
  ...
  if(bits<=8) {
    ...
    printf("const unsigned char %s[0x%04x*3+16] = {\n",filename,bmpsize);
    printf(" 0x%02x,0x%02x,0x%02x,0x%02x, // width\n", bmp[0x12],bmp[0x12+1],bmp[0x12+2],bmp[0x12+3]);
    printf(" 0x%02x,0x%02x,0x%02x,0x%02x, // height\n", bmp[0x16],bmp[0x16+1],bmp[0x16+2],bmp[0x16+3]);
    printf(" 0x%02x,0x%02x, // bits\n",bmp[0x1C], bmp[0x1C+1]);
    printf(" 0x%02x,0x%02x,0x%02x,0x%02x, // reserv\n", 0, 0);
    printf(" 0x%02x,0x%02x, // offset\n", 0, 16);
  }
  //printf("offs=0x%04x\n",offs);
  printf("  ");
  int wl = w, ww = w;
