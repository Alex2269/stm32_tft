...
{
  adcResult += read_adc(ADC1, 8); // read adc channel 8, pin PA8 // Сумируем полученные значения ацп

  buff_ray[i] = tft_H-45-adcResult/18; // Вычисляем среднее значение
  real_result[i] = adcResult*0.82; // 3300/4096=0.80566 Вычисляем среднее значение
...
void write_text(uint16_t i) // for text data
{
  ...
  tft_puts8x16(4,110,(int8_t*)lbl_real,TFT_STRING_MODE_BACKGROUND);
