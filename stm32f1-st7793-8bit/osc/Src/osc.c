...
void trigger_on(uint16_t trig)
{
  ...
  if(chanal_b)
  {
    fill_buff(0); b1 = real_result_b[0];
    fill_buff(0); b2 = real_result_b[0];
    
    while (!((b1 < trig) && (b2 >= trig))) // т.е. было выше порога, а стало ниже или равно - был переход через порог
    {
        count++;
        b1 = b2;
        fill_buff(0); b2 = real_result_b[0];
...
void fill_buff(uint16_t i)
{
  ...
  while(adc_counter < adc_speed) // while counter measure less N
  {
    ...
    */
    adcResult_a += (float)HAL_ADC_GetValue(&hadc1); // Сумируем полученные значения ацп
    adcResult_b += (float)HAL_ADC_GetValue(&hadc2); // Сумируем полученные значения ацп
    // --
    adc_counter++; // Увеличиваем счетчик измерений
  }

  adc_tmp_a = tft_H - (int16_t) (buff_scroll_y + (adcResult_a / adc_counter / 20)); // Вычисляем среднее значение
  adc_tmp_b = tft_H - (int16_t) (buff_scroll_y + (adcResult_b / adc_counter / 20)); // Вычисляем среднее значение

  real_result_a[i] = adc_tmp_a * 0.82; // 3300/4096=0.80566 Вычисляем среднее значение
  real_result_b[i] = adc_tmp_b * 0.82; // 3300/4096=0.80566 Вычисляем среднее значение

  if(adc_tmp_a < 24) adc_tmp_a = 24;
  if(adc_tmp_a > 216) adc_tmp_a = 216;

  if(adc_tmp_b < 24) adc_tmp_b = 24;
  if(adc_tmp_b > 216) adc_tmp_b = 216;

  buff_ray_a[i] = adc_tmp_a; // Вычисляем среднее значение
  buff_ray_b[i] = adc_tmp_b; // Вычисляем среднее значение
  ...
  {
    #define M_PI 3.14159
    #define RAY_A_POS_Y 96
    #define RAY_B_POS_Y 120
    buff_ray_a[i] = (tft_H - buff_scroll_y) + (uint16_t) 96*cos(30*0.05*M_PI*i/180)+RAY_A_POS_Y;
    buff_ray_b[i] = (tft_H - buff_scroll_y) + (uint16_t) 64*sin(30*0.05*M_PI*i/180)+RAY_B_POS_Y;
  }
  #endif
}
...
void rotate_buff(uint16_t r)
{
  ...
  for(uint16_t i = 0; i < r; i++) // scroll buff_ray_a
  {
    ...
    if(chanal_b)
    {
      tmp_b = buff_ray_b[WIDTH_RAY-1];
      for(uint16_t i = WIDTH_RAY-1; i; i--)
      {
        buff_ray_b[i] = buff_ray_b[i-1];
      }
      buff_ray_b[0] = tmp_b;
    }
  }
