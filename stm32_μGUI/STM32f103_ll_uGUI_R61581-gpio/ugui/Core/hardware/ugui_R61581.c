...
void Display_Init(void)
{
  ...
  UG_Init(&gui_R61581, Display_PSet, get_width(), get_heihgt());
...
{
  if((x < 0) ||(x >= get_width()) || (y < 0) || (y >= get_heihgt())) return;
...
UG_RESULT HW_FillFrame(UG_S16 x1, UG_S16 y1, UG_S16 x2, UG_S16 y2, UG_COLOR c)
{
  ...
  if((x1 < 0) ||(x1 >= get_width()) || (y1 < 0) || (y1 >= get_heihgt())) return UG_RESULT_FAIL;
  if((x2 < 0) ||(x2 >= get_width()) || (y2 < 0) || (y2 >= get_heihgt())) return UG_RESULT_FAIL;
