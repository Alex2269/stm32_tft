...
void draw_window(void)
{
  ...
  // DMA_FillFrame(2, 2, 800-4, 480-4, INDIGO);
  ...
  #define yset1 112
  #define yset2 yset1+164
  drawRoundRect(5, yset1+1, Display_Get_WIDTH()-10,  yset2+8, 8, RED);
  drawRoundRect(6, yset1+2, Display_Get_WIDTH()-12,  yset2+6, 8, GREEN);
  drawRoundRect(7, yset1+3, Display_Get_WIDTH()-14,  yset2+4, 8, CYAN);
...
void draw_dynamic_grid(int16_t shiftx)
{
  ...
  for(uint16_t y = y_pos;y<y_pos+(ceil_x*8);y+= 8)
    for(uint16_t x = shiftx; x<ceil_x+shiftx; x+=32) fillRect(x, y, 2,2, CYAN);
  for(uint16_t y = y_pos;y<y_pos+(ceil_x*8);y+=32)
...
void graph_line(uint16_t xpos)
{
  ...
  buff_1_draw[xpos] = read_adc(ADC1, GPIOA, 0x1UL << 4)/14+126;
  buff_2_draw[xpos] = read_adc(ADC1, GPIOA, 0x1UL << 5)/14+128;
  buff_3_draw[xpos] = read_adc(ADC1, GPIOA, 0x1UL << 6)/14+130;

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_color(xpos, buff_1_draw, buff_1_clean, 3, CYAN,INDIGO);
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_color(xpos, buff_2_draw, buff_2_clean, 3, YELLOW,INDIGO);
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_color(xpos, buff_3_draw, buff_3_clean, 3, GREEN,INDIGO);
  osMutexRelease(Display_MutexHandle); // mutex off
  osDelay(5);
}
...
void draw_row_level(void)
{
  ...
  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_dot_line(x_offset + space_width * 0, y_offset, x_offset + space_width * 0, y_offset - clear0, level_width, DARKSLATEGRAY);
  write_graph_dot_line(x_offset + space_width * 0, y_offset, x_offset + space_width * 0, y_offset - val0, level_width, OLIVE);
  clear0 = val0;
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_dot_line(x_offset + space_width * 1, y_offset, x_offset + space_width * 1, y_offset - clear1, level_width, DARKSLATEGRAY);
  write_graph_dot_line(x_offset + space_width * 1, y_offset, x_offset + space_width * 1, y_offset - val1, level_width, SILVER);
  clear1 = val1;
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_dot_line(x_offset + space_width * 2, y_offset, x_offset + space_width * 2, y_offset - clear2, level_width, DARKSLATEGRAY);
  write_graph_dot_line(x_offset + space_width * 2, y_offset, x_offset + space_width * 2, y_offset - val2, level_width, TEAL);
  clear2 = val2;
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_dot_line(x_offset + space_width * 3, y_offset, x_offset + space_width * 3, y_offset - clear3, level_width, DARKSLATEGRAY);
  write_graph_dot_line(x_offset + space_width * 3, y_offset, x_offset + space_width * 3, y_offset - val3, level_width, GREEN);
  clear3 = val3;
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_dot_line(x_offset + space_width * 4, y_offset, x_offset + space_width * 4, y_offset - clear4, level_width, DARKSLATEGRAY);
  write_graph_dot_line(x_offset + space_width * 4, y_offset, x_offset + space_width * 4, y_offset - val4, level_width, CYAN);
  clear4 = val4;
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_dot_line(x_offset + space_width * 5, y_offset, x_offset + space_width * 5, y_offset - clear5, level_width, DARKSLATEGRAY);
  write_graph_dot_line(x_offset + space_width * 5, y_offset, x_offset + space_width * 5, y_offset - val5, level_width, MAGENTA);
  clear5 = val5;
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_dot_line(x_offset + space_width * 6, y_offset, x_offset + space_width * 6, y_offset - clear6, level_width, DARKSLATEGRAY);
  write_graph_dot_line(x_offset + space_width * 6, y_offset, x_offset + space_width * 6, y_offset - val6, level_width, ORANGE);
  clear6 = val6;
  osMutexRelease(Display_MutexHandle); // mutex off

  osMutexWait(Display_MutexHandle, portMAX_DELAY); // mutex on
  write_graph_dot_line(x_offset + space_width * 7, y_offset, x_offset + space_width * 7, y_offset - clear7, level_width, DARKSLATEGRAY);
