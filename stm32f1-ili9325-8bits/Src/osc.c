...
void oscil_init(void)
{
  ...
  tft_fill(1,1,30,get_width()-1,COLOR_YELLOW); // draw statusbar

  tft_draw_rect(get_height()-31,0,get_height(),get_width(),COLOR_RED); // draw contour statusbar
  tft_fill(get_height()-30,1,get_height()-1,get_width()-1,COLOR_YELLOW); // draw statusbar
...
void oscil_run(void)
{
    ...
    */
    ...
    for(uint16_t i = 0; i < get_width(); i++) // measurement
    {
      fill_buff(i);
    }

    for(uint16_t i = 0; i < get_width(); i++) // display redraw
    {
      write_graph(i);
    }
...
void fill_buff(uint16_t i)
{
  ...
  buff_ray[i] = get_height()-45-adcResult/adc_counter/18; // Вычисляем среднее значение
...
{
  for(uint16_t x = 32;x<get_height()-31;x+=8)for(uint16_t i = 16; i<get_width()-15; i+=64)tft_draw_point(i, x, COLOR_GRAYBLUE); //draw vertical dot line
  for(uint16_t x = 32;x<get_height()-31;x+=64)for(uint16_t i = 16; i<get_width()-15; i+=8)tft_draw_point(i, x, COLOR_WHITE); //draw horizontal dot line
}
